...
export function GameTimeCalculatorScreen({ players }: { players: Player[] }) {
  ...
  const [extraSubstitutions, setExtraSubstitutions] = useState(false);
  ...
  const plan = canCreatePlan ? createGameTimePlan({
    extraRotationMinutes: [
      ...(subDuringPlay ? [matchMinutes - 5] : []),
      ...(forceEqualTime && extraSubstitutions ? midRotationPoints(matchMinutes, rotationMinutes) : [])
    ],
    forceEqualTime,
    matchMinutes,
  ...
  const shuffleStartingGroup = () => setStartingPlayer((current) => availablePlayers.length < 2 ? 0 : (current + 1 + Math.floor(Math.random() * (availablePlayers.length - 1))) % availablePlayers.length);

  return (
    <View style={styles.container}>
      <View style={styles.availabilityStrip}>
        <View style={styles.availabilityCopy}><Text style={styles.availabilityValue}>{availablePlayers.length}</Text><Text style={styles.availabilityText}>available players from your saved squad</Text></View>
        <Pressable accessibilityLabel="Shuffle starting group" disabled={!canCreatePlan} onPress={shuffleStartingGroup} style={[styles.shuffleButton, !canCreatePlan && styles.shuffleButtonDisabled]}><Text style={styles.shuffleButtonText}>Shuffle start</Text></Pressable>
      </View>
      <View style={styles.settingsCard}>
        ...
        <View style={styles.toggleRow}>
          <View style={styles.toggleCopy}><Text style={styles.toggleTitle}>Extra substitutions for equality</Text><Text style={styles.toggleHint}>Adds a midpoint switch in each rotation only when you want an exact equal-time plan.</Text></View>
          <Switch accessibilityLabel="Extra substitutions for equality" disabled={!forceEqualTime} onValueChange={setExtraSubstitutions} trackColor={{ false: "#b9b4a9", true: "#78925d" }} value={forceEqualTime && extraSubstitutions} />
        </View>
      </View>
      {plan ? <Plan availablePlayers={availablePlayers} forceEqualTime={forceEqualTime} isSharing={isSharing} matchMinutes={matchMinutes} onShare={() => void sharePlan()} plan={plan} playersOnPitch={activePlayersOnPitch} shareCardRef={shareCardRef} spread={spread} /> : <EmptyPlan />}
    </View>
  );
}

type PlanProps = { availablePlayers: Player[]; forceEqualTime: boolean; isSharing: boolean; matchMinutes: number; onShare: () => void; plan: ReturnType<typeof createGameTimePlan>; playersOnPitch: number; shareCardRef: RefObject<View | null>; spread: number };

function Plan({ availablePlayers, forceEqualTime, isSharing, matchMinutes, onShare, plan, playersOnPitch, shareCardRef, spread }: PlanProps) {
  return <><View style={styles.resultCard}><View style={styles.resultHeader}><View><Text style={styles.resultTitle}>{forceEqualTime ? "Fair game time" : "Rotation plan"}</Text><Text style={styles.target}>{plan.targetMinutes.toFixed(1)} min</Text><Text style={styles.targetCaption}>{forceEqualTime ? "average planned time" : "average scheduled time"}</Text></View><View style={styles.fairnessBadge}><Text style={styles.fairnessValue}>{spread} min</Text><Text style={styles.fairnessLabel}>maximum gap</Text></View></View><Text style={styles.resultText}>{forceEqualTime ? "Players with the lowest scheduled time are selected first at every rotation." : "Players rotate in squad order. Turn on equal time to make the allocation fairer."}</Text></View><View style={styles.section}><Text style={styles.sectionTitle}>Player allocation</Text><View style={styles.allocationGrid}>{plan.minutesByPlayer.map((minutes, index) => <View key={availablePlayers[index].id} style={styles.playerChip}><Text numberOfLines={1} style={styles.playerName}>{availablePlayers[index].name.trim() || `Player ${index + 1}`}</Text><Text style={styles.playerMinutes}>{minutes} min</Text></View>)}</View></View><View style={styles.section}><Text style={styles.sectionTitle}>Rotation plan</Text><Text style={styles.sectionHint}>Use each row as your on-pitch group for that block.</Text><View style={styles.schedule}>{plan.periods.map((period) => <View key={period.start} style={styles.period}><Text style={styles.periodTime}>{period.start}'-{period.end}'</Text><Text style={styles.periodPlayers}>{period.players.map((player) => availablePlayers[player].name.trim() || `Player ${player + 1}`).join("  |  ")}</Text></View>)}</View></View><View style={styles.shareSection}><View collapsable={false} ref={shareCardRef} style={styles.shareCard}><Text style={styles.shareBrand}>GRASSROOTS FC</Text><Text style={styles.shareTitle}>{forceEqualTime ? "FAIR GAME TIME" : "ROTATION PLAN"}</Text><Text style={styles.shareMeta}>{matchMinutes} min  |  {playersOnPitch} on pitch  |  {plan.targetMinutes.toFixed(1)} min average</Text><View style={styles.shareDivider} /><Text style={styles.shareHeading}>PLAYER ALLOCATION</Text>{plan.minutesByPlayer.map((minutes, index) => <View key={availablePlayers[index].id} style={styles.shareRow}><Text style={styles.shareName}>{availablePlayers[index].name.trim() || `Player ${index + 1}`}</Text><Text style={styles.shareMinutes}>{minutes} min</Text></View>)}<Text style={styles.shareHeading}>ROTATIONS</Text>{plan.periods.map((period) => <View key={period.start} style={styles.sharePeriod}><Text style={styles.sharePeriodTime}>{period.start}'-{period.end}'</Text><Text style={styles.sharePeriodPlayers}>{period.players.map((player) => availablePlayers[player].name.trim() || `Player ${player + 1}`).join(" / ")}</Text></View>)}</View><Pressable accessibilityLabel="Share rotation plan" disabled={isSharing} onPress={onShare} style={[styles.shareButton, isSharing && styles.shareButtonDisabled]}><Text style={styles.shareButtonText}>{isSharing ? "Preparing graphic..." : "Share rotation plan"}</Text></Pressable></View></>;
}
...
function midRotationPoints(matchMinutes: number, rotationMinutes: number) { return Array.from({ length: Math.ceil(matchMinutes / rotationMinutes) }, (_, index) => { const start = index * rotationMinutes; const end = Math.min(start + rotationMinutes, matchMinutes); return start + (end - start) / 2; }).filter((point) => point > 0 && point < matchMinutes); }

const styles = StyleSheet.create({
  allocationGrid: { flexDirection: "row", flexWrap: "wrap", gap: 8 }, availabilityCopy: { alignItems: "center", flex: 1, flexDirection: "row", gap: 10 }, availabilityStrip: { alignItems: "center", backgroundColor: "#e5ebc7", borderRadius: 12, flexDirection: "row", gap: 10, padding: 15 }, availabilityText: { color: "#375134", flex: 1, fontSize: 13, fontWeight: "700" }, availabilityValue: { color: "#18321f", fontFamily: "Avenir Next Condensed", fontSize: 31, fontWeight: "800" }, container: { gap: 22 }, fairnessBadge: { alignItems: "flex-end", gap: 2 }, fairnessLabel: { color: "#d0d9c3", fontSize: 11 }, fairnessValue: { color: "#ffffff", fontSize: 18, fontWeight: "800" }, noticeCard: { backgroundColor: "#f8f5ed", borderColor: "#ded7c8", borderRadius: 12, borderWidth: 1, gap: 4, padding: 18 }, noticeText: { color: "#626d60", fontSize: 14, lineHeight: 20 }, noticeTitle: { color: "#1a2a1e", fontSize: 16, fontWeight: "800" }, period: { borderBottomColor: "#ded7c8", borderBottomWidth: StyleSheet.hairlineWidth, flexDirection: "row", gap: 16, paddingVertical: 13 }, periodPlayers: { color: "#1a2a1e", flex: 1, fontSize: 14, fontWeight: "700", lineHeight: 20 }, periodTime: { color: "#6c7566", fontSize: 13, fontWeight: "700", width: 58 }, playerChip: { backgroundColor: "#e5ebc7", borderRadius: 9, gap: 2, minWidth: "30%", paddingHorizontal: 12, paddingVertical: 10 }, playerMinutes: { color: "#18321f", fontSize: 16, fontWeight: "800" }, playerName: { color: "#5a6856", fontSize: 11, fontWeight: "700" }, resultCard: { backgroundColor: "#19382a", borderRadius: 14, gap: 14, padding: 20 }, resultHeader: { alignItems: "flex-start", flexDirection: "row", justifyContent: "space-between" }, resultText: { color: "#dce4d5", fontSize: 14, lineHeight: 20, maxWidth: 310 }, resultTitle: { color: "#dce8b1", fontSize: 15, fontWeight: "800" }, schedule: { backgroundColor: "#f8f5ed", borderRadius: 12, paddingHorizontal: 16 }, section: { gap: 10 }, sectionHint: { color: "#626d60", fontSize: 14, lineHeight: 20 }, sectionTitle: { color: "#1a2a1e", fontFamily: "Avenir Next Condensed", fontSize: 23, fontWeight: "800", letterSpacing: -0.3 }, settingsCard: { backgroundColor: "#f8f5ed", borderColor: "#ded7c8", borderRadius: 12, borderWidth: 1, gap: 2, padding: 18 }, shareBrand: { color: "#dce8b1", fontSize: 11, fontWeight: "900", letterSpacing: 1.4 }, shareButton: { alignItems: "center", backgroundColor: "#19382a", borderRadius: 10, paddingVertical: 13 }, shareButtonDisabled: { backgroundColor: "#829083" }, shareButtonText: { color: "#ffffff", fontSize: 14, fontWeight: "800" }, shareCard: { backgroundColor: "#19382a", borderRadius: 14, gap: 10, padding: 18 }, shareDivider: { backgroundColor: "#54715b", height: StyleSheet.hairlineWidth }, shareHeading: { color: "#dce8b1", fontSize: 11, fontWeight: "900", letterSpacing: 1 }, shareMeta: { color: "#d7e2d2", fontSize: 12, lineHeight: 18 }, shareMinutes: { color: "#ffffff", fontSize: 13, fontWeight: "800" }, shareName: { color: "#e8eee4", flex: 1, fontSize: 13, fontWeight: "700" }, sharePeriod: { flexDirection: "row", gap: 10 }, sharePeriodPlayers: { color: "#e8eee4", flex: 1, fontSize: 11, lineHeight: 16 }, sharePeriodTime: { color: "#dce8b1", fontSize: 11, fontWeight: "800", width: 45 }, shareRow: { flexDirection: "row", gap: 8 }, shareSection: { gap: 10 }, shareTitle: { color: "#ffffff", fontFamily: "Avenir Next Condensed", fontSize: 29, fontWeight: "800", letterSpacing: -0.4 }, shuffleButton: { backgroundColor: "#19382a", borderRadius: 8, paddingHorizontal: 10, paddingVertical: 8 }, shuffleButtonDisabled: { backgroundColor: "#829083" }, shuffleButtonText: { color: "#ffffff", fontSize: 11, fontWeight: "800" }, stepper: { alignItems: "center", borderBottomColor: "#ded7c8", borderBottomWidth: StyleSheet.hairlineWidth, flexDirection: "row", justifyContent: "space-between", paddingVertical: 14 }, stepperButton: { alignItems: "center", backgroundColor: "#e5ebc7", borderRadius: 9, height: 36, justifyContent: "center", width: 36 }, stepperButtonText: { color: "#18321f", fontSize: 21, fontWeight: "700", lineHeight: 23 }, stepperControls: { alignItems: "center", flexDirection: "row", gap: 10 }, stepperLabel: { color: "#26352a", fontSize: 15, fontWeight: "700" }, stepperValue: { color: "#18321f", fontSize: 14, fontWeight: "800", minWidth: 54, textAlign: "center" }, target: { color: "#ffffff", fontFamily: "Avenir Next Condensed", fontSize: 38, fontWeight: "800", letterSpacing: -1 }, targetCaption: { color: "#d0d9c3", fontSize: 12, marginTop: -2 }, toggleCopy: { flex: 1, gap: 2 }, toggleHint: { color: "#687164", fontSize: 12, lineHeight: 17, maxWidth: 240 }, toggleRow: { alignItems: "center", flexDirection: "row", gap: 16, paddingTop: 14 }, toggleTitle: { color: "#26352a", fontSize: 15, fontWeight: "800" }
});
