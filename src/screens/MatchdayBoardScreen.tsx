...
export function MatchdayBoardScreen({
  ...
  const updateField = (
    field: "focus" | "kickoff" | "matchDate" | "opponent" | "venue",
    value: string,
  ...
  return (
    <View style={styles.container}>
      <View style={styles.hero}>
        ...
        <Text style={styles.heroTitle}>Match plan</Text>
        <Text style={styles.heroHint}>
          {hasLoadedMatchday
            ? "Set the detail. Keep the game simple."
            : "Loading your saved matchday board..."}
        </Text>
      </View>
      ...
      <View style={styles.details}>
        <TextInput
          ...
          onChangeText={(value) => updateField("opponent", value)}
        ...
        <View style={styles.detailRow}>
          <Field
            ...
            placeholder="Sat 23 Aug"
          />
          <Field
            label="Kick-off"
            value={draft.kickoff}
            onChangeText={(value) => updateField("kickoff", value)}
            placeholder="10:30"
          />
        </View>
        <Field
      ...
      <View style={styles.focusCard}>
        ...
        <TextInput
          accessibilityLabel="Coaching focus"
          multiline
          onChangeText={(value) => updateField("focus", value)}
          placeholder="e.g. Be brave receiving the ball, then play forward."
          placeholderTextColor="#7b8a79"
      ...
      <View style={styles.shareSection}>
        <View collapsable={false} ref={shareCardRef} style={styles.shareCard}>
          ...
          <View style={styles.shareMeta}>
            <Text style={styles.shareMetaItem}>
              {draft.kickoff.trim() || "Kick-off TBC"}
            </Text>
            <Text style={styles.shareMetaItem}>
              {draft.venue.trim() || "Venue TBC"}
            </Text>
          </View>
          <View style={styles.shareRule} />
          <Text style={styles.shareFocusLabel}>TODAY'S FOCUS</Text>
          <Text style={styles.shareFocus}>
            {draft.focus.trim() || "Play with confidence and enjoy the game."}
          </Text>
          <Text style={styles.shareSquadLabel}>
            SQUAD ({availablePlayers.length})
          </Text>
          <Text style={styles.shareSquad}>
            {availablePlayers.length
              ? availablePlayers
                  .map((player) => player.name.trim() || "Player")
                  .join("  ·  ")
              : "Availability to be confirmed"}
          </Text>
        </View>
        <Pressable
          accessibilityLabel="Share match plan"
          disabled={isSharing}
        ...
        >
          <Text style={styles.shareButtonText}>
            {isSharing ? "Preparing image..." : "Share match plan"}
          </Text>
        </Pressable>
      </View>
      ...
      <View style={styles.section}>
        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>Squad ready</Text>
          <Text style={styles.count}>{availablePlayers.length} available</Text>
        </View>
        <View style={styles.players}>
          {availablePlayers.length ? (
            ...
            <Text style={styles.empty}>
              Set player availability in Squad before the match.
            </Text>
          )}
        </View>
      </View>
      ...
      <View style={styles.section}>
        <View style={styles.sectionHeader}>
          ...
          <Text style={styles.count}>
            {completedChecks}/{draft.checks.length}
          </Text>
        </View>
        <View style={styles.checklist}>
          {draft.checks.map((check) => (
            <Pressable
              ...
              accessibilityState={{ checked: check.done }}
              onPress={() =>
                setDraft((current) => ({
                  ...current,
                  checks: current.checks.map((item) =>
                    item.id === check.id ? { ...item, done: !item.done } : item,
                  ),
                }))
              }
              style={styles.check}
            >
              <View style={[styles.box, check.done ? styles.boxDone : null]}>
                {check.done ? <Text style={styles.tick}>✓</Text> : null}
              </View>
              <Text
                style={[
                  styles.checkLabel,
                  check.done ? styles.checkLabelDone : null,
                ]}
              >
                {check.label}
              </Text>
...
const styles = StyleSheet.create({
  ...
  boxDone: { backgroundColor: "#f06a2f", borderColor: "#f06a2f" },
  check: {
    alignItems: "center",
    flexDirection: "row",
    gap: 11,
  ...
  container: { gap: 22 },
  count: { color: "#b34a20", fontSize: 12, fontWeight: "800" },
  date: { color: "#f4b08b", fontSize: 13, fontWeight: "700" },
  ...
  focusTitle: { color: "#7b3519", fontSize: 15, fontWeight: "800" },
  hero: { backgroundColor: "#173a2a", borderRadius: 5, gap: 4, padding: 20 },
  ...
  tick: { color: "#ffffff", fontSize: 15, fontWeight: "900" },
});
